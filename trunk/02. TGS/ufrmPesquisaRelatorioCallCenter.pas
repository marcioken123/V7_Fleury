unit ufrmPesquisaRelatorioCallCenter;
...
type
  ...
  TQueryParams = record
                    ...
                    ChoosePeriodoDoRelatorio, ChooseDuracao,
                      ...
                      ChoosePeriodoDoDia, ChooseFilas, ChooseTags: boolean;
                    IniPswd, EndPswd,
  ...
  TfrmPesquisaRelatorioCallCenter = class(TfrmPesquisaRelatorioBase)
  ...
  public
    ...
    vlRepVars : record
                   Tipos, Atendentes, PAs, Filas, Tags, Senhas, PeriodoDoRelatorio, PeriodoDoDia, Duracao, MultiUnidades : string;
...
function frmSicsPesquisaRelatorioCallCenter(const aIDUnidade: integer; const aAllowNewInstance: Boolean = False; const aOwner: TComponent = nil): TfrmPesquisaRelatorioCallCenter;
...
function frmSicsPesquisaRelatorioCallCenter(const aIDUnidade: integer; const aAllowNewInstance: Boolean = False; const aOwner: TComponent = nil): TfrmPesquisaRelatorioCallCenter;
begin
  Result := TfrmPesquisaRelatorioCallCenter(TfrmPesquisaRelatorioCallCenter.GetInstancia(aIDUnidade, aAllowNewInstance, aOwner));
end;

procedure TfrmPesquisaRelatorioCallCenter.CarregarTreeViews;
begin
...
procedure TfrmPesquisaRelatorioCallCenter.CarregaUnidades;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          begin
            if FieldByName('CONECTADA').AsBoolean <> dmUnidades.cdsUnidades.FieldByName('CONECTADA').AsBoolean then
            begin
              Edit;
              FieldByName('CONECTADA').AsBoolean := dmUnidades.cdsUnidades.FieldByName('CONECTADA').AsBoolean;
              if ((not FieldByName('CONECTADA').AsBoolean) and (FieldByName('ID').AsInteger <> ID_UNIDADE_PADRAO)) then
                FieldByName('SELECIONADO').AsBoolean := False;
              Post;
            end;
          end;

          if (FieldByName('CONECTADA').AsBoolean) and (not FieldByName('CARREGOU_DADOS').AsBoolean) then
          begin
            Edit;
...
procedure TfrmPesquisaRelatorioCallCenter.cbTipo1Change(Sender: TObject);
begin
  inherited;
   vlChanged := true;
...
procedure TfrmPesquisaRelatorioCallCenter.edMaiorMenorChange(Sender: TObject);
begin
  vlChanged := true;
...
procedure TfrmPesquisaRelatorioCallCenter.edEntreChange(Sender: TObject);
begin
  vlChanged := true;
  (FindComponent('rbTipo'+inttostr((Sender as TEdit).Tag)+'Entre') as TRadioButton).IsChecked := true;
end;

procedure TfrmPesquisaRelatorioCallCenter.EditarClientDataSet(const tabela: string;
...
procedure TfrmPesquisaRelatorioCallCenter.rbEntreMouseUp(Sender: TObject;
...
procedure TfrmPesquisaRelatorioCallCenter.rbMaiorMenorMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  vlChanged := true;
...
procedure TfrmPesquisaRelatorioCallCenter.rbTipo1TodosChange(Sender: TObject);
begin
  inherited;
   vlChanged := true;
end;

procedure TfrmPesquisaRelatorioCallCenter.SetIDUnidade(const Value: Integer);
begin
...
procedure TfrmPesquisaRelatorioCallCenter.IniPeriodCheckBoxChange(Sender: TObject);
begin
  inherited;
   vlChanged := true;
...
procedure TfrmPesquisaRelatorioCallCenter.DayPeriodCheckBoxChange(Sender: TObject);
begin
  inherited;
   vlChanged := true;
...
procedure TfrmPesquisaRelatorioCallCenter.PasswordCheckBoxChange(Sender: TObject);
begin
  inherited;
   vlChanged := true;
...
procedure TfrmPesquisaRelatorioCallCenter.PosicionaUnidadeCorrente;
begin
  inherited;
end;

procedure TfrmPesquisaRelatorioCallCenter.GetQueryParams;
var
...
procedure TfrmPesquisaRelatorioCallCenter.MontarWhere(QryEventos: TFDQuery; var SqlInTags: string; IgnoreDuracao: Boolean);
...
begin
  ...
  if ((vlQueryParams.ChoosePeriodoDoRelatorio = false) and
      ...
      (vlQueryParams.ChoosePeriodoDoDia = false)) then
  begin
         ...
         begin
           ...
           begin
             ...
             if sCampoOrigem <> 'TAGS' then
              ...
              s := s + 'not exists (select id_ticket from nn_tickets_tags where nn_tickets_tags.id_unidade = e.id_unidade and nn_tickets_tags.id_ticket = e.id_ticket where nn_tickets_tags.id_unidade = :id_unidade)';
           ...
           begin
             ...
             begin
               SqlInTags := sIn;
               s := s + 'exists (select id_ticket from nn_tickets_tags where nn_tickets_tags.id_unidade = :ID_UNIDADE and nn_tickets_tags.id_unidade = e.id_unidade and nn_tickets_tags.id_ticket = e.id_ticket and nn_tickets_tags.id_tag in (' + sIn + '))';
             end;
           end;
           s := s + ')and';
...
procedure TfrmPesquisaRelatorioCallCenter.cdsUnidadesBeforeInsert(DataSet: TDataSet);
begin
   if not FPermiteInserirUnidades then Abort;
end;

procedure TfrmPesquisaRelatorioCallCenter.GetRepVars;
var
  i, iDetalhes, iTotalRec: Integer;
  bAtdsDivergente, bPAsDivergente, bFilasDivergente, bTagsDivergente, bTodos, bTbNenhum: Boolean;
  sAtdsBase, sPAsBase, sFilasBase, sTagsBase, sIds, sCampoOrigem, sOldIndexFieldNames, sIdsTemp: string;
...
procedure TfrmPesquisaRelatorioCallCenter.btnExibirTabelaClick(Sender: TObject);
begin
...
procedure TfrmPesquisaRelatorioCallCenter.btnExibirGraficosTMETMAClick(Sender: TObject);
var
...
procedure TfrmPesquisaRelatorioCallCenter.CancelButtonClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmPesquisaRelatorioCallCenter.AtendsTreeViewKeyDown(Sender: TObject;
...
procedure TfrmPesquisaRelatorioCallCenter.BitBtn1Click(Sender: TObject);
begin
  if CheckUnidadesSelecionadas then
    ExibirGraficoSLA;
end;

procedure TfrmPesquisaRelatorioCallCenter.ExibirGraficoSLA;
begin
  if not cbTipo1.IsChecked then
  begin
    InformationMessage('Selecione uma opção de filtro (Espera)');
    Exit;
  end;
...
procedure TfrmPesquisaRelatorioCallCenter.FormClose(Sender: TObject;
...
procedure TfrmPesquisaRelatorioCallCenter.FormResize(Sender: TObject);
const
  OFF = 20;
begin
  inherited;
...
function TfrmPesquisaRelatorioCallCenter.CheckUnidadesSelecionadas: Boolean;
begin
  Result := True;
  ...
  begin
    if dmUnidades.cdsUnidadesClone.Locate('ID', cdsUnidades.FieldByName('ID').Value, []) then
    begin
      if cdsUnidades.FieldByName('CONECTADA').AsBoolean <>
         dmUnidades.cdsUnidadesClone.FieldByName('CONECTADA').AsBoolean then
      begin
        cdsUnidades.Edit;
        cdsUnidades.FieldByName('CONECTADA').AsBoolean :=
          dmUnidades.cdsUnidadesClone.FieldByName('CONECTADA').AsBoolean;
        cdsUnidades.Post;
      end;
    end;
...
constructor TfrmPesquisaRelatorioCallCenter.Create(AOwner: TComponent);
var
  i  : integer;
begin
  FfrmSicsReport := nil;
...
procedure TfrmPesquisaRelatorioCallCenter.dbgrdUnidadesCheckFieldClick(
  Field: TField);
begin
...
procedure TfrmPesquisaRelatorioCallCenter.dbgrdUnidadesDrawColumnCell(Sender: TObject;
  const Canvas: TCanvas; const Column: TColumn; const Bounds: TRectF;
  const Row: Integer; const Value: TValue; const State: TGridDrawStates);
begin
  inherited;
...
procedure TfrmPesquisaRelatorioCallCenter.dbgrdUnidadesKeyDown(Sender: TObject;
  var Key: Word; var KeyChar: Char; Shift: TShiftState);
begin
  inherited;
   //
...
destructor TfrmPesquisaRelatorioCallCenter.Destroy;
begin
  inherited;
end;
