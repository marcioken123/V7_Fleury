unit APIs.Aspect.Sics.AtualizarAtendimento;
...
type
  TAPIAspectSicsAtualizarAtendimento = class
  private
  public
    type
...
procedure PopularParametros;
begin
  TAPIAspectSicsAtualizarAtendimento.URL := controller.parametros.CarregarParametroStr('APIs.Aspect.Sics', 'URL_AtualizarAtendimento', 'http://localhost/aspect/rest/AA/AtualizarAtendimento');
end;

class function TAPIAspectSicsAtualizarAtendimento.Execute(const AParametrosEntrada: TParametrosEntradaAPI; out AParametrosSaida: TParametrosSaidaAPI; out AErroAPI: TErroAPI): TTipoRetornoAPI;
var
  LJSON: TJSONObject;
  LRetorno: string;
  LJSONResposta: TJSONObject;
begin
  TLogSQLite.New(tmInfo, clResponse, 'Consumindo API AtualizarAtendimento').Save;
  TLog.MyLogTemp('Consumindo API AtualizarAtendimento', nil, 0, False, TCriticalLog.tlINFO);
  ...
  try
    try
      ...
      LJSON.AddPair('AIdFluxo', AParametrosEntrada.IdFluxo);
      LJSON.AddPair('AIdFila', AParametrosEntrada.IdFila);
      LJSON.AddPair('AQtdPessoas', AParametrosEntrada.QtdPessoas);
      LJSON.AddPair('ASenha', AParametrosEntrada.Senha);
