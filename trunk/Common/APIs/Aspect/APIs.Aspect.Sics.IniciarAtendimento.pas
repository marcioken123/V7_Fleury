unit APIs.Aspect.Sics.IniciarAtendimento;
...
type
  TAPIAspectSicsIniciarAtendimento = class
  private
  public
    type
...
procedure PopularParametros;
begin
  TAPIAspectSicsIniciarAtendimento.URL := controller.parametros.CarregarParametroStr('APIs.Aspect.Sics', 'URL_IniciarAtendimento', 'http://localhost/aspect/rest/AA/IniciarAtendimento');
end;

class function TAPIAspectSicsIniciarAtendimento.Execute(const AParametrosEntrada: TParametrosEntradaAPI; out AParametrosSaida: TParametrosSaidaAPI; out AErroAPI: TErroAPI): TTipoRetornoAPI;
var
  LJSON: TJSONObject;
  LRetorno: string;
  LJSONResposta: TJSONObject;
begin
  TLogSQLite.New(tmInfo, clResponse, 'Consumindo API IniciarAtendimento').Save;
  TLog.MyLogTemp('Consumindo API IniciarAtendimento', nil, 0, False, TCriticalLog.tlINFO);
  LJSON := TJSONObject.Create;
  try
    try
      LJSON.AddPair('AIdUnidade', AParametrosEntrada.IdUnidade);
      LJSON.AddPair('AIdTotem', AParametrosEntrada.IdTotem);
